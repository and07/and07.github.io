...
if (isIe)
{
	...
	win.addEventListener = function(event, handler, flag) {
	...
	if (win.dialogArguments && win.dialogArguments.external) {
	    extern = win.dialogArguments.external;
	...
	XPathException = new _XPathExceptionHandler();
	...
	// DOMException
	// An Error object will be thrown, this is just a handler to instantiate that object
	DOMException = new _DOMExceptionHandler();
	...
	{
		// return XPathResult
		return doc.createExpression(expression, resolver).evaluate(contextNode, type, result);
	}
			...
			)
	{
		...
		{
			return doc._XPathMsxmlDocumentHelper.documentChangeDetected();
		}
...
//			return ((node = getSingleNode(result)) ? (node.nodeType == 1 ? node.innerText : node.nodeValue) : null);
		...
		function findNode(result, current)
		{
			switch(current.nodeType)
			{
				...
				case 2: // NODE_ATTRIBUTE
					...
					if (id)
					{
						var node = doc.getElementById(id.text);
						if (node)
						{
				...
				case 3: // NODE_TEXT
					...
					if (id)
					{
						var node = doc.getElementById(id.value);
						if (node)
						{
		...
		function activateResult(result)
		{
			if (!result._domResult)
			{
				try
				{
					...
					if (result._contextNode != doc && expression.indexOf("//") != 0)
					{
					...
					{
						result._domResult = doc._XPathMsxmlDocumentHelper.getDom().selectNodes(expression);
					}
					else
					{
						result._domResult = true;
						result._textResult = doc._XPathMsxmlDocumentHelper.getTextResult(expression);
					}
	...
	doc.reloadDom = function()
	{
		doc._XPathMsxmlDocumentHelper.reset();
	}

	doc._XPathMsxmlDocumentHelper = new _XPathMsxmlDocumentHelper();
	function _XPathMsxmlDocumentHelper()
	{
		...
		{
			doc._propertyChangeDetected = true;
		}
		
		this.documentChangeDetected = function()
		{
			return (doc.ignoreDocumentChanges ? false : this._currentElementCount != doc.all.length || doc._propertyChangeDetected);
		}
		...
		function activateDom(helper)
		{
			if (!helper.dom)
			{
				...
				helper.dom = dom;
				helper._currentElementCount = doc.all.length;
				doc._propertyChangeDetected = false;
			}
			else
			{
		...
		{
			return loadNode(dom, dom, doc.body, helper);		
		}
		...
		function loadNode(dom, domParentNode, node, helper)
		{
			...
			else
			{
                ...
                if (name != '#comment')
                {
    				var domNode = dom.createElement(name);
    				if (!node.id)
    				{
